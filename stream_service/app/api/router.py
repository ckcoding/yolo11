...
from app.core.engine import push_stream_worker
from app.core.events import active_websockets
from app.core.pipeline import execute_pipeline, normalize_pipeline
from app.core.pool import model_pool
from app.core.state import active_tasks
from app.core.storage import persist_image_array
...
def resolve_stream_source(source: str) -> str:
    ...
    parsed = urlparse(raw)
    if parsed.scheme in {"http", "https", "rtmp", "rtsp", "udp", "tcp", "srt", "ws", "wss"}:
        return raw
    if parsed.scheme == "file":
...
async def detect_stream(req: StreamRequest):
    ...
    stream_url = resolve_stream_source(stream_url)
...
async def websocket_endpoint(websocket: WebSocket, task_id: str):
    ...
    from app.core.events import MAX_WS_PER_TASK
    if len(active_websockets[task_id]) >= MAX_WS_PER_TASK:
        await websocket.close(code=1013, reason="该任务 WebSocket 连接数已达上限")
        return
